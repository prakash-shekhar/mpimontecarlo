...
void input(int rank, int world, int *numvars, int *pointer_iterations);
void mpi_type(int *pointer_iterations, MPI_Datatype *mpi_input);
...
double f(double xinputs[N])
{
    ...
    return pow(xinputs[0], 3) + sin(xinputs[1]) + pow(2, xinputs[2]);
}
...
void mpi_type(int *pointer_iterations, MPI_Datatype *mpi_input)
{
    MPI_Aint addresss_iterations;
    MPI_Aint displacement_arr[1] = {0};
    int blocklengths_arr[1] = {1};
    MPI_Datatype types_arr[1] = {MPI_INT};
    ...
    MPI_Type_create_struct(1, blocklengths_arr, displacement_arr, types_arr, mpi_input);
...
void input(int rank, int world, int *numvars, int *pointer_iterations)
{
    ...
    mpi_type(pointer_iterations, &mpi_input);
